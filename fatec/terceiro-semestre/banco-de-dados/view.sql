...
FROM
    consulta AS C
    JOIN paciente P ON C.codigo = P.codigo
    JOIN medico M ON C.codigo = M.crm
WHERE
...
FROM
    consulta AS C
    JOIN convenio Co ON C.codigo = Co.codigo
WHERE
    Co.nome = 'Amil'
    AND C.dia_hora IN (
        '2014-04-01',
...
FROM
    ...
    JOIN consulta C ON CE.codigo_consulta = C.codigo
    JOIN medico M ON C.codigo = M.crm
WHERE
    M.nome = 'Dr. Eduardo'
